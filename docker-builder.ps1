...
Function Get-LastContainerVersionFromFile {
    $version_file_exists = Test-Path -Path "./version.txt"

    if(!$version_file_exists) {
        return "0.0.0"
    }

    $tag = Get-Content -Path "./version.txt"
    $tag = $tag.Trim();
...
Function Build-Docker {
    ...
    # version.txt
    cmd.exe /c docker build . -t ${ProjectName}:${ImageVersion} -f $DockerfilePath
    cmd.exe /c docker save ${ProjectName}:${ImageVersion} -o ./${ProjectName}-${ImageVersion}.tar

    Set-Content -Path "./version.txt" -Value $ImageVersion
    Write-Host -ForegroundColor Green "Image successfully published at registry.tristesse.lol/p/masi-integratedproject/containers/${ProjectName}:$ImageVersion`r`n"
}
...
Build-Docker -ProjectName $ProjectName -ImageVersion $ImageVersion -Dockerfilepath $DockerfilePath.FullName
